...
func FindNotificationApi(c *fiber.Ctx) error {
	...
	if notification.UserId != userId && role != "admin" {
		return utils.SendErrorResponse(c, fiber.StatusForbidden, "Not authorized to access this notification")
	}

	return utils.SendDataResponse(c, notification)
}
