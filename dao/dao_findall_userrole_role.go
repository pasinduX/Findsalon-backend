...
func FindAllUserRolesByRole(role string, skip int64, limit int64) ([]dto.UserRole, int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	collection := dbConfig.DATABASE.Collection(dbConfig.USERROLES_COLLECTION)
	filter := bson.M{"Role": role, "Deleted": false}
	...
	roles := []dto.UserRole{}
	for cursor.Next(ctx) {
		var roleItem dto.UserRole
		if err := cursor.Decode(&roleItem); err != nil {
			continue
		}
